...
A separate line of work takes benchmarks, rather than models, as the object of study. A systematic study of 60 language model benchmarks defines benchmark saturation and reports that nearly half of them show it, at rates that rise with benchmark age, with expert curation rather than the privacy of test data associated with resilience \citep{akhtar2026plateau}. Sample-level auditing annotates individual items of MMLU, ARC, WinoGrande, HellaSwag, and TruthfulQA and reports within-benchmark variation that aggregate accuracy does not show \citep{siedler2026benchmarks}. A reference-free judging framework rates conversational-agent benchmarks on consistency, complexity, and policy coverage \citep{koren2026benchmarking}. Applying safety benchmarks built for larger models to smaller ones produces rankings that move with the treatment of ambiguous responses \citep{shaik2026benchmarking}. Inverse-density weighting reduces the influence of benchmark multiplicity on an aggregate score \citep{lin2026balance}. A position argument holds that an evaluation score should carry its evidential scope and validity window, and that aggregation should be conservative when component signals differ in reliability \citep{gilda2026position}. These are recent entries in that literature rather than a survey of it.

Such methods judge benchmark quality, annotate benchmark items, reweight benchmark suites, or set rules for combining evaluation evidence. Benchmark RADAR does not score benchmark quality, re-rank models, or prescribe an aggregation rule. It retains source records with their evidence and makes that population searchable, so that work of this kind has a discoverable set of candidates and their provenance to start from. A saturation analysis, for instance, needs a population of benchmarks with release dates and score histories before it can measure anything \citep{akhtar2026plateau}. The census in Section~\ref{sec:results} reports which records expose scale, date, and protocol metadata, part of what such an analysis would check before comparing measurements. We assembled this paragraph with Benchmark RADAR's command-line client, following the retrieval and inspection procedure of Section~\ref{sec:worked-example}, and read each candidate at its source.
...
The study does not measure retrieval precision, task suitability, or time saved. The worked example combines local queries with web search and has no controlled baseline. Lexical matching can miss paraphrases and renamed tasks. Daily discovery is also a forward crawl. Preprints enter through a feed of new submissions, so a paper first posted before the collection window stays outside the corpus even when a later version appears inside it, and no increase in corpus size recovers it. The retrieval described in Section~\ref{sec:related-work} returned neither of two relevant earlier studies \citep{yang2026benchmark,akhtar2026plateau}; both were added after a coauthor identified and read them. A missing result documents the limits of the collection procedure and the query, not the absence of the work from the literature. Evaluating semantic retrieval \citep{reimers2019sentencebert} will require reviewed relevance judgments across queries and candidate records, including less familiar benchmark families.

The scale and date audit identifies metadata gaps. The full-catalog score, document, and date analyses retain measurements across all sources, without establishing benchmark saturation or score stagnation. Saturation has been defined and tested directly elsewhere, against benchmark age and design properties \citep{akhtar2026plateau}. Establishing those outcomes here requires source review for comparable test versions and settings, and dates tied to actual score reporting or evaluation. Matching archive labels cannot verify independent repeated runs.
