...
The v0.11.0 release includes a frozen data archive and checksums matching the recorded audit inputs. To reproduce from source, check out software commit \gitcommit{}, install its development dependencies, and run the six-step CI sequence. Run the software's \texttt{scripts/export\_report\_figure\_data.py} and its \texttt{--check} mode to refresh \texttt{figure-data.tex}. From the paper checkout, run \texttt{python scripts/audit\_catalog.py /path/to/software}, then repeat with \texttt{--check}. The README records the commands. Finally, run \texttt{make arxiv}, compile the extracted package, and inspect the rebuilt PDF. Normal paper builds use the committed data and do not refresh the evidence.

The DOI in Table~\ref{tab:access-artifacts} identifies the archived v0.9.0 paper. This manuscript describes the v0.11.0 release at software commit \gitcommit{} and the cutoff above. The dashboard continues to change; citations of its live counts need a retrieval date.
