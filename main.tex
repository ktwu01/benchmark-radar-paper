...
Benchmark RADAR complements these methods by making benchmark records, available measurements, and provenance searchable. Its contribution is discovery and evidence organization; benchmark-quality assessment, model ranking, and aggregation require subsequent analysis. Saturation studies illustrate this relationship: release dates and score histories provide evidence for examining how benchmark performance changes with age \citep{akhtar2026plateau}. Our census reports the available evidence and its gaps, including the scale, date, and protocol metadata needed to assess comparability (Appendix~\ref{app:measurement-coverage}). We identified literature for this section through Benchmark RADAR's command-line client and coauthor recommendations, then read the source papers, following the retrieval and inspection procedure in Section~\ref{sec:worked-example}.
...
Historical coverage is a further limitation. The arXiv discovery route collects new submissions; it does not backfill papers first posted before the collection window when later versions appear. The searches used for Section~\ref{sec:related-work} missed two relevant earlier studies \citep{yang2026benchmark,akhtar2026plateau}, which a coauthor identified and read. This example motivates historical retrieval alongside daily collection when assembling related work.

The full-catalog score, document, and date analyses characterize evidence coverage and metadata gaps. Assessing benchmark saturation or score stagnation would require comparable test versions and settings, together with dates tied to score reporting or evaluation. \citet{akhtar2026plateau} provide a direct study of saturation in relation to benchmark age and design properties. Extending our analysis in this direction would require source review to establish comparability and verify independent repeated runs beyond matching archive labels.
