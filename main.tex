...
% Explicit labels keep the same author-affiliation relationships in PDF and HTML.
\renewcommand{\Authfont}{\normalfont\large}
\renewcommand{\Affilfont}{\normalfont\fontsize{7.5}{8.5}\selectfont}
\renewcommand{\Authsep}{\quad}
\renewcommand{\Authand}{\quad}
\renewcommand{\Authands}{\quad}
\setlength{\affilsep}{0.35em}
% Only change authblk's PDF separator so affiliations wrap compactly; the
% individual labeled declarations below remain the source for both formats.
\makeatletter
\def\AB@affilsepx{\protect\quad\protect\Affilfont}
\makeatother
\author[1,2]{Koutian~Wu\thanks{Corresponding author: \href{mailto:k@tacite.ai}{k@tacite.ai}}}
\author[3]{Junjie~Zhou}
\author[4]{Ergan~Shang}
\author[5]{Jiayu~Wang}
\author[6]{Pengqian~Han}
\author[7]{Junkai~Wang}
\author[8]{Wanghan~Xu}
\author[9]{Songyuanyi~Lu}
\author[10]{Lin~Shi}
\affil[1]{Earth-Space-AI}
\affil[2]{Tacite~AI}
\affil[3]{Hangzhou~Dianzi~University}
\affil[4]{Carnegie~Mellon~University}
\affil[5]{Xi'an~Jiaotong~University}
\affil[6]{The~University~of~Auckland}
\affil[7]{Tsinghua~University}
\affil[8]{Shanghai~Jiao~Tong~University}
\affil[9]{The~University~of~Hong~Kong}
\affil[10]{Cornell~Tech}
\date{}
...
Figure~\ref{fig:taxonomy-trends} focuses on benchmark release or first-introduction dates from 2023~H1, grouped by half-year and Level~1 domain. The 132 earlier records remain in the catalog but are omitted because historical coverage is incomplete. Over 2023~H2--2025~H2, agentic evaluation exhibits the largest share increase among the displayed series, whereas knowledge and factuality, image-based evaluation, and mathematical reasoning have lower pooled shares in the later periods. These patterns describe the collected catalog. Incomplete and varying source coverage precludes interpreting them as field-wide release rates or the smaller trailing bars as declining benchmark activity.

\RadarFigure
  {taxonomy-trends.pdf}
  {Temporal distribution and changing composition of benchmark records. Counts refer to source records, not independent benchmark releases. (A) Release or first-introduction dates by half-year and Level~1 domain from 2023~H1: 936 dated records and 215 undated records, totaling \ReportNumber{1151} records in the legend. Undated records use the same vertical scale as the dated records; 132 earlier records are outside the displayed window. The final period ends at the \datacutoff{} cutoff. (B) Shares of 6 selected domains and facets among all dated records in each period during 2023~H2--2025~H2. These periods contain at least 30 dated records and have source compositions close to the pooled catalog. The selected series represent large changes in pooled share. Legend changes, in percentage points, compare the earlier and later halves of the displayed periods, excluding the middle period. Facets overlap domains, so the plotted shares do not sum to 100\%.}
  {fig:taxonomy-trends}
...
For the temporal analysis in Figure~\ref{fig:taxonomy-trends}, \ReportNumber{1068} of the \ReportNumber{\CensusRecords} source records have documented benchmark release or first-introduction dates, while 215 remain undated. Across score observations, \ReportNumber{\FindingsModelDateScores} entries carry model-announcement dates and \FindingsDocumentDateScores{} carry document-publication dates. Neither date basis directly establishes an evaluation date. Researchers must first establish dates and comparable evaluation settings for the relevant source records.
